...
#
# Case 1 only misbehaves when the operand the stale register points at is a
# named local or a parameter -- with a constant, a global or a call result the
# stale register happens to be the right one -- so the helpers below take
# parameters instead of using literals.
#
# Case 3 needs a left-hand side that requires code to be loaded: an index, a
# member, a global or an upvalue. A local on the left is unaffected because it
# needs no load.
